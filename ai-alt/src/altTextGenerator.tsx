...
export function AltTextGeneratorTool() {
  ...
  useEffect(() => {
    loadSettings();
    fetchImagesWithoutAltText(false); // Don't show toast on initial load
  }, []);

  useEffect(() => {
    if (apiKey && !loadingModels && models.length === DEFAULT_MODELS.length) {
  ...
  const loadSettings = async () => {
    try {
      ...
      const settings = await client.fetch(
        `*[_id == "altTextPluginSettings"][0]{ apiKey, model, globalContext, recentContexts }`,
      );
      ...
      if (settings && settings.apiKey) {
        ...
        console.log("Settings loaded successfully:", {
          ...
          hasGlobalContext: !!settings.globalContext,
          recentContextsCount: settings.recentContexts?.length || 0,
        });
      } else {
      ...
      toast.push({
        ...
        description: "Your previous settings may not be available",
      });
    }
  };
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      // Use createOrReplace to handle both create and update cases
      const result = await client.createOrReplace({
        _type: "altTextPluginSettings",
        _id: "altTextPluginSettings",
        apiKey,
        model,
        globalContext,
        recentContexts,
      ...
      toast.push({
        ...
        description: error.message || "Failed to save settings. Check console for details.",
      });
    }
  };
  ...
  const fetchAvailableModels = async () => {
    if (!apiKey) {
      toast.push({
        status: "warning",
        title: "API Key Required",
        description: "Please enter your OpenAI API key to fetch available models",
      });
      return;
    }
    ...
    try {
      ...
      // Filter for standard vision-capable GPT-4 models only
      const visionModels = data.data
        .filter((model: any) => {
          const id = model.id.toLowerCase();
          // Only include standard GPT-4 models with vision capability
          return (
            (id.startsWith("gpt-4o") ||
              id.startsWith("gpt-4-turbo") ||
              id === "gpt-4-vision-preview") &&
            // Exclude fine-tuned, audio, realtime, and other specialized models
            !id.includes("audio") &&
        ...
        .map((model: any) => ({
          value: model.id,
          title: model.id.replace(/-/g, " ").replace(/\b\w/g, (l: string) => l.toUpperCase()),
        }))
      ...
      if (visionModels.length > 0) {
        ...
        toast.push({
          ...
          description: `Found ${visionModels.length} vision-capable model${visionModels.length !== 1 ? "s" : ""}`,
        });
      } else {
        ...
        toast.push({
          ...
          description: "No vision models found, using defaults",
        });
      }
    } catch (error: any) {
  ...
  const fetchImagesWithoutAltText = async (showToast = true) => {
    setLoading(true);
    try {
      const query = `*[_type == "sanity.imageAsset" && !defined(altText)] | order(_createdAt desc) {
        _id,
        url,
        altText,
        metadata,
      ...
      setSelectedImages(new Set()); // Clear selection when refreshing
  ...
  const generateAltText = async (imageAsset: ImageAsset, context?: string) => {
    if (!apiKey) {
    ...
    // Combine global context with batch-specific context
    const batchContext = context || contextText || "";
    const combinedContext = [globalContext, batchContext].filter(Boolean).join("\n\n");
  ...
  const updateImageAltText = async (imageId: string, altText: string) => {
    await client.patch(imageId).set({ altText }).commit();
  };
  ...
  const processSelectedImages = async () => {
    ...
    if (imagesToProcess.length === 0) {
      toast.push({
        ...
        description: "Please select images to process or use 'Generate All'",
      });
      return;
    }
    ...
    for (let i = 0; i < imagesToProcess.length; i++) {
      ...
      try {
        const altText = await generateAltText(image, contextText);
        newGeneratedTexts.set(image._id, {
          imageId: image._id,
          altText,
          status: "pending",
        });
    ...
    if (successCount > 0) {
      ...
      // Save context to recent contexts if it was used
      ...
      toast.push({
        ...
        description: `Generated alt text for ${successCount} image${
          successCount !== 1 ? "s" : ""
        }${errorCount > 0 ? `. ${errorCount} failed.` : ""}. Review and commit below.`,
      });
    } else {
      toast.push({
        status: "error",
  ...
  const regenerateAltText = async (imageId: string) => {
  ...
  const updateGeneratedAltText = (imageId: string, newAltText: string) => {
    setGeneratedAltTexts((prev) => {
      const next = new Map(prev);
  ...
  const commitAllAltTexts = async () => {
    ...
    if (textsToCommit.length === 0) {
      toast.push({
        ...
        description: "Accept some alt text entries before committing",
      });
      return;
    }
    ...
    for (const item of textsToCommit) {
      try {
        await updateImageAltText(item.imageId, item.altText);
        successCount++;
      } catch (error: any) {
        errorCount++;
    ...
    if (successCount > 0) {
      toast.push({
        ...
        description: `Successfully saved ${successCount} alt text${
          successCount !== 1 ? "s" : ""
      ...
      toast.push({
        status: "error",
        title: "Commit Failed",
        description: "Failed to save alt text. Check console for details.",
      });
    }
  };
  ...
  const saveContextToRecent = async (context: string) => {
    ...
    // Add to recent contexts, keeping only last 5 unique entries
            ...
            <Flex align="center" gap={3}>
              ...
              {processing ? (
                <>
                  <Spinner />
                  <Text size={1}>
                    Generating alt text... {processedCount} of {totalCount} complete
                  </Text>
                </>
              ) : (
        ...
        <Card borderBottom padding={4} tone="transparent">
          <Container width={2}>
            <Stack space={4}>
              ...
              <Stack space={3}>
                <Stack space={2}>
                  <Label size={1}>OpenAI API Key</Label>
                  <TextInput
                    type="password"
                    placeholder="sk-..."
                    value={apiKey}
                ...
                <Stack space={2}>
                  ...
                  <TextArea
                    placeholder="Add default context that will be applied to all images (e.g., ‘Images from a design portfolio by the studio SOUP. Alt texts should focus on the specific design and materiality of the featured element(s) within the image’')"
                    value={globalContext}
                    onChange={(e) => {
                  ...
                  />
                  <Text size={0} muted>
                    This context will be applied to all alt text generation. You can add
                    batch-specific context when processing images.
                  </Text>
                </Stack>
              </Stack>
              ...
              <Flex gap={2}>
                ...
                {settingsSaved && (
                  <Badge
                    tone="positive"
                    fontSize={1}
                    style={{ alignContent: "center", justifyItems: "center", minWidth: "3rem" }}
                  >
                    ✓
                  </Badge>
                )}
              ...
              <Flex direction="column" align="center" justify="center" gap={3}>
                ...
                <Stack space={2}>
                  ...
                  <Text size={1} muted align="center">
                    Great job on accessibility! All your images are properly described.
                  </Text>
                </Stack>
              </Flex>
            </Card>
            ...
            <Stack space={4}>
                ...
                {images.map((image) => {
                  ...
                  return (
                    <Flex direction="column" gap={2} justify="flex-end" key={image._id}>
                      ...
                      <Flex align="center" gap={2} style={{textOverflow: "ellipsis", width: "80%"}}>
                        <Checkbox
                          checked={isSelected}
              ...
              {!apiKey && (
                <Card padding={3} radius={2} tone="caution">
                  <Text size={1}>
                    ⚠️ Please configure your OpenAI API key in settings to generate alt text
                  </Text>
                </Card>
              )}
            </Stack>
            ...
            <Stack space={4}>
              <Stack space={3}>
                <Stack space={2}>
                  ...
                  <TextArea
                    placeholder="Add specific context for this batch of images (e.g., ‘These images are scans from a newspaper called “The Evening Soup”’)"
                    value={contextText}
                  ...
                  {globalContext && (
                    <Badge size={0} tone="caution" style={{ width: "max-content" }}>
                      Global context from settings will also be included in this prompt
                    </Badge>
                  )}
                </Stack>
              ...
              <Flex gap={2} justify="space-between">
                ...
                <Flex gap={2}>
                  ...
                  <Button
                    text={
                      selectedImages.size > 0 ? `Generate (${selectedImages.size})` : "Generate All"
                    }
                    tone="primary"
                    icon={SparklesIcon}
                    onClick={() => {
        ...
        >
          <Box padding={4}>
            <Stack space={4}>
              <Flex align="center" justify="space-between">
                <Badge tone="primary">
                  {generatedAltTexts.size} image{generatedAltTexts.size !== 1 ? "s" : ""} with generated alt text
                </Badge>
                          ...
                          <Flex align="flex-start" justify="space-between" gap={2}>
                            <Stack space={2} flex={1}>
                              ...
                              <TextArea
                                ...
                                onChange={(e) =>
                                  updateGeneratedAltText(imageId, e.currentTarget.value)
                                }
                                rows={3}
                                fontSize={1}
                              />
                            ...
                            {!isAccepted ? (
                              <>
                                ...
                                <Button
                                  ...
                                  disabled={regeneratingImageId === imageId || committing}
                                  onClick={() => regenerateAltText(imageId)}
                                />
                                <Button
                                  text="Decline"
                                  mode="ghost"
